...
- enabled [private project memory](../../tui/project-memory/), including its
  backend policy and bounded `MEMORY.md` index
- `AGENTS.md` or legacy `KOLEGA.md` project guidance
- deprecated repository `AGENT_MEMORY.md`, loaded read-only as separately
  labelled repository-controlled context

These sections remain distinct and survive coder, planning, and sub-agent prompt
overrides. They are also different from resumable session history and the
Planning tab's transient task list. Project memory is not generated from either:
Kolega Code performs no completed-turn transcript review or automatic memory
authoring.
...
deprecated repository `AGENT_MEMORY.md` separately continues to withhold
probable-secret content. Disabling project memory removes both its context and
tools without deleting its private data.
