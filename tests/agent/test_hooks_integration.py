...
async def test_parallel_batch_denies_only_matching_tool(tmp_path, agent_config, monkeypatch):
    ...
    # read is parallel-safe, so the batch runs via asyncio.gather.
    monkeypatch.setattr(agent, "tool_collection", _tools(handler, name="read", parallel_safe=True))

    results = await agent.process_tool_calls(
        [_call(name="read", index=1, task="boom"), _call(name="read", index=2, task="ok")]
    )
