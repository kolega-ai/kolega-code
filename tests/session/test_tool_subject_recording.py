...
async def test_tool_subject_recording_roundtrip(
    ...
    await emitter.chat(
        ...
        tool_call_id="c",
        tool_subject="src/original.py",
        tool_display={"paths": ["src/original.py"]},
    )
    await emitter.emit(
        AgentEvent(
            sender="agent",
    ...
    await emitter.chat(
        ...
        "finished",
        tool_call_id="c",
        tool_subject=result_subject,
        tool_display={"paths": [result_subject]} if result_subject else None,
    )
    ...
    assert stored[0].content["tool_display"] == {"paths": ["src/original.py"]}
    assert "tool_subject" not in stored[1].content
    assert "tool_display" not in stored[1].content
    if result_subject is None:
        assert "tool_subject" not in stored[2].content
        assert "tool_display" not in stored[2].content
    else:
        assert stored[2].content["tool_subject"] == result_subject
        assert stored[2].content["tool_display"] == {"paths": [result_subject]}
    assert all("tool_subject" not in event.content for event in stored[3:])
    assert all("tool_display" not in event.content for event in stored[3:])
    assert inner.broadcast_event.await_count == 5

    allowed_chat_keys = {"message_type", "text", "tool_description", "tool_call_id", "tool_subject", "tool_display"}
    ...
    assert chat_events[0]["content"]["tool_display"] == {"paths": ["src/original.py"]}
    ...
    assert items[0].tool_display == {"paths": [result_subject or "src/original.py"]}
    assert items[0].status == status
    assert items[0].text == "finished"
    assert items[1].tool_subject is None
    assert items[1].tool_display is None
    ...
    assert serialized[0]["tool_display"] == {"paths": [result_subject or "src/original.py"]}
    assert "tool_display" not in serialized[1]


@pytest.mark.parametrize("delegated", [False, True])
def test_projection_keeps_late_multiline_details_and_ignores_malformed_updates(delegated: bool) -> None:
    info = {"dispatch_id": "d1", "agent_name": "runner"} if delegated else None
    command = "echo first\n  echo second"
    events = [
        AgentEvent(
            sender="runner",
            ...
            sub_agent_info=info,
            content={"message_type": "tool_call", "tool_description": "exec_command", "tool_call_id": "t"},
        ),
        AgentEvent(
            sender="runner",
            event_type="tool_streaming_update",
            sub_agent_info=info,
            content={"tool_call_id": "t", "tool_display": {"command": command}, "text": "partial"},
        ),
        AgentEvent(
            sender="runner",
            event_type="chat_message",
            ...
            content={
                ...
                "tool_call_id": "t",
                "tool_display": {"command": ["not", "a", "command"], "arguments": {"raw": "NEVER_DISPLAY"}},
                "text": "result",
            },
        ),
    ]
    state = replay(events)
    item = (state.sub_agents["d1"].steps if delegated else state.conversation)[0]
    assert item.tool_display == {"command": command}
    encoded = state.to_dict()
    serialized = encoded["sub_agents"]["d1"]["steps"] if delegated else encoded["conversation"]
    assert serialized[0]["tool_display"] == {"command": command}
    assert "NEVER_DISPLAY" not in json.dumps(encoded)
